<?php
    ...
    if (isset($_POST["submit"])){
        ...
        if (!empty($_POST["name"]) && !empty($_POST["comment"])) {
                ...
                $stmt=$dbh->prepare("insert into Responses (title,res_body,res_userid,thread_id) values(?,?,?,?)");
                $stmt->execute(array($_POST["name"],$_POST["comment"],$_SESSION["userid"],$_POST["res_thread"]));
...
<body>
    ...
    <?php echo htmlspecialchars($errorMessage, ENT_QUOTES); ?></font></div>
        ...
        <p>
            ...
            <input type="hidden" name="res_thread" value=<?php echo htmlspecialchars($thread_id);?>>
        </form>
